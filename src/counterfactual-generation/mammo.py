...
class EmbedDataModule(BaseDataModuleClass):
    ...
    def get_all_csv_from_config(
        ...
        print("elfs",self.config.data.prop_train < 1.0)
        ...
        test_df = df.loc[df.empi_anon.isin(test_id)]

        ptest_df = test_df[test_df["ManufacturerModelName"] == "Selenia Dimensions"]
        #test_df = test_df.head(5000)
        #test_df["image_path"].to_csv("embed_test_ids.csv", index=False)
